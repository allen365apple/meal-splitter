...
const BillSplitter = () => {
    ...
    const removeTag = (tagId) => {
        ...
        if (affected > 0 && !confirm(`「${tag ? tag.emoji + ' ' + tag.name : '這個標籤'}」目前有 ${affected} 位成員。\n\n刪除後他們會被改成「${tags.find(t => t.id !== tagId).name}」，相關菜色會變成「自訂名單」。\n\n確定要刪除嗎？`)) return;
        pushUndo(`已刪除標籤「${tag ? tag.name : ''}」`);
    ...
    const addUser = () => {
        ...
        setUsers([...users, { id: nextId, name: `朋友 ${nextId}`, tagId: tags[0].id }]);
    ...
    const removeUser = (id) => {
        ...
        if (orphaned.length > 0) {
            ...
            const ok = confirm(`刪掉這個人之後，下面這些菜會變成沒有人分攤：\n\n${names}\n\n這些金額會被標成「未分配」，不會算進任何人的帳單。\n\n確定要刪除嗎？`);
            if (!ok) return;
        }
        pushUndo(`已刪除成員「${userName(id)}」`);
    ...
    const selectGroup = (targetTagId) => {
        ...
        if (groupUserIds.length === 0) {
            ...
            alert(`「${tag ? tag.emoji + ' ' + tag.name : '這個標籤'}」目前沒有成員喔！\n\n請先回上一步，把成員的標籤改成這一個。`);
            return;
        }
    ...
    const removeItem = (id) => {
        ...
        pushUndo(`已刪除「${target ? target.name : '項目'}」`);
    ...
    const parseImportLine = (rawLine) => {
        ...
        if (!name) return { skipped: true, raw, reason: '沒有品名' };
    ...
    const handleCalcEqual = () => {
        ...
        if (r === null) { setCalcExpression('算式有誤'); setTimeout(() => setCalcExpression(''), 1200); return; }
    ...
    const copyToClipboard = (userId, uName, data) => {
        ...
        let text = `🧾 ${uName} 的帳單明細\n----------------\n`;
        data.details.forEach(d => {
            const div = (d.totalUnits === d.sharersCount) ? `${d.sharersCount}人` : `${d.totalUnits}份`;
        ...
        if (data.extra !== 0) text += `固定加收: ${fmt(data.extra)}\n`;
        text += `💰 應付總額: $${data.total}`;
        if (data.isTreated) text += `\n🎁 ${data.treatedByIds.map(userName).join('、')} 請客`;
        else if (payerId && payerId !== userId && data.total > 0) text += `\n\n👉 請轉帳 $${data.total} 給 ${userName(payerId)}`;
        executeCopy(text, () => { setCopiedId(userId); setTimeout(() => setCopiedId(null), 2000); });
    };
    const copyAllResults = () => {
        const fmt = (v) => (v < 0 ? `-$${Math.abs(v)}` : `$${v}`);
        let text = `🍻 聚餐友善分帳機 - 結算單\n📅 日期: ${new Date().toLocaleDateString()}\n💰 總金額: $${calculation.grandTotal} (含 ${serviceChargeRate}% 服務費`;
        if (calculation.extraTotal !== 0) text += `、固定加收 $${calculation.extraTotal}`;
        text += `)\n`;
        // 只寫一句話就好。下面每個人的金額就是要轉的數字，前面再列一次是重複資訊。
        if (payerId && calculation.userResults[payerId]) {
            text += `\n💳 ${userName(payerId)} 先付，請大家轉帳給他\n`;
        }
        text += `----------------------------\n`;
        users.forEach(u => {
            ...
            if (data.serviceCharge !== 0) parts.push(`服務費(${fmt(data.serviceCharge)})`);
            if (data.extra !== 0) parts.push(`固定加收(${fmt(data.extra)})`);
            text += `   └ 明細: ${parts.join(' + ')}\n`;
            // 請客只是註記，不動任何人的金額，所以寫在明細後面
            if (data.isTreated) text += `   └ 🎁 ${data.treatedByIds.map(userName).join('、')} 請客\n`;
        });
        if (calculation.hasUnassigned) text += `\n⚠️ 還有 $${Math.abs(calculation.unassigned)} 沒有分配給任何人（${calculation.unassignedItems.join('、')}）\n`;
        text += `\n----------------------------\n由「聚餐友善分帳機」自動計算\nDesigned by 王柏文`;
    ...
    const renderUserTagSelector = () => (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-fade-in" onClick={() => setEditingUser(null)}>
            <div className="bg-white rounded-xl shadow-xl w-3/4 max-w-xs overflow-hidden" onClick={e => e.stopPropagation()}>
                <div className="p-4 bg-slate-50 border-b border-slate-100 font-bold text-slate-700">選擇標籤</div>
                ...
                <div className="p-3 border-t border-slate-100"><button onClick={() => { setEditingUser(null); setShowTagManager(true); }} className="w-full py-2 bg-slate-800 text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2"><Settings size={16}/> 管理 / 新增標籤</button></div>
            </div>
        </div>
    );
    ...
    const renderTagManager = () => (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm m-4 overflow-hidden flex flex-col max-h-[80vh]">
                <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50"><h3 className="font-bold text-lg text-slate-800 flex items-center gap-2"><Settings size={20}/> 管理群組標籤</h3><button onClick={() => setShowTagManager(false)} aria-label="關閉" title="關閉" className="p-2 text-slate-400 hover:text-slate-600 bg-white rounded-full border border-slate-200"><X size={20}/></button></div>
                <div className="p-4 overflow-y-auto flex-1">
                    <div className="space-y-3 mb-6">{tags.map(tag => (<div key={tag.id} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-xl shadow-sm"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-2xl">{tag.emoji}</div><span className="font-bold text-slate-700">{tag.name}</span></div>{tags.length > 1 && <button onClick={() => removeTag(tag.id)} aria-label={`刪除標籤 ${tag.name}`} title="刪除標籤" className="p-2 text-slate-300 hover:text-red-500"><Trash2 size={18}/></button>}</div>))}</div>
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200"><h4 className="text-sm font-bold text-slate-500 mb-3 uppercase tracking-wider">新增自訂標籤</h4><div className="flex gap-2 mb-3"><div className="relative"><select value={newTagEmoji} onChange={e => setNewTagEmoji(e.target.value)} aria-label="選擇標籤圖示" className="h-full px-2 bg-white border border-slate-300 rounded-lg text-xl appearance-none outline-none focus:border-green-500">{['🍺','🍷','🥩','🥗','👶','🍰','🚗','🎤','🎲','🐶'].map(e => <option key={e} value={e}>{e}</option>)}</select><span className="absolute right-1 top-1/2 -translate-y-1/2 pointer-events-none text-xs text-slate-400">▼</span></div><input type="text" placeholder="群組名稱 (如: 喝酒)" className="flex-1 px-3 py-2 border border-slate-300 rounded-lg outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500" value={newTagName} onChange={e => setNewTagName(e.target.value)}/></div><button onClick={addTag} disabled={!newTagName} className="w-full py-2 bg-green-500 text-white rounded-lg font-bold hover:bg-green-600 disabled:bg-slate-300 disabled:shadow-none shadow-md transition-all"><Plus size={18} className="inline mr-1"/> 新增標籤</button></div>
                </div>
            </div>
        </div>
    );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm m-4 overflow-hidden flex flex-col h-[75vh]">
                <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2"><PasteIcon size={20}/> 貼上文字明細</h3>
                    <button onClick={() => setShowImportModal(false)} aria-label="關閉" title="關閉" className="p-2 text-slate-400 hover:text-slate-600 bg-white rounded-full border border-slate-200"><X size={20}/></button>
                </div>
                <div className="p-4 flex-1 flex flex-col gap-4 overflow-y-auto">
                    ...
                    <div className="flex-1 flex flex-col">
                        <div className="flex justify-between items-end mb-1">
                            <label className="text-sm font-bold text-slate-700">在此貼上明細</label>
                            <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setShowInstallGuide(false)}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm m-4 overflow-hidden" onClick={e => e.stopPropagation()}>
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">📲 安裝到主畫面</h3>
                    <button onClick={() => setShowInstallGuide(false)} aria-label="關閉" title="關閉" className="p-2 text-slate-400 hover:text-slate-600 bg-white rounded-full border border-slate-200"><X size={20}/></button>
                </div>
                <div className="p-6 space-y-6">
                    ...
                    <div className="flex gap-4 items-start">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-sm text-slate-500">
                                1. 點擊下方 <span className="inline-block bg-slate-100 p-1 rounded"><ShareIcon size={14} className="inline"/> 分享</span> 按鈕<br/>
                                2. 往下滑，選擇 <span className="font-bold text-slate-700">加入主畫面</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 items-start">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-sm text-slate-500">
                                1. 點擊右上角 <span className="inline-block bg-slate-100 p-1 rounded"><MoreVertical size={14} className="inline"/> 選單</span> 按鈕<br/>
                                2. 選擇 <span className="font-bold text-slate-700">安裝應用程式</span> 或 <span className="font-bold text-slate-700">加入主畫面</span>
                            </p>
                        </div>
                    </div>
                </div>
                <div className="p-4 bg-green-50 text-center">
                    <p className="text-xs text-green-700 font-medium">✨ 這樣就能像 APP 一樣全螢幕使用囉！</p>
                </div>
            </div>
        </div>
    );
    ...
    const renderCalculatorModal = () => (
        ...
        <div className="bg-white w-full max-w-sm rounded-t-2xl sm:rounded-2xl shadow-2xl p-4 flex flex-col gap-3">
            <div className="flex justify-between items-center mb-2"><h3 className="font-bold text-slate-700 flex items-center gap-2"><CalculatorIcon size={20}/> 輔助計算機</h3><button onClick={() => setShowCalculator(false)} aria-label="關閉計算機" title="關閉" className="p-2 text-slate-400 hover:text-slate-600"><X size={24}/></button></div>
            ...
            <button onClick={confirmCalcResult} className="mt-2 w-full bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-green-200 active:scale-95">確認金額</button>
        </div></div>
    );
                ...
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-start">
                        <div className="min-w-0">
                            <h3 className="font-bold text-lg text-slate-800 truncate">⚖️ {item.name} 的份數</h3>
                            <p className="text-xs text-slate-500 mt-1">
                                {item.price < 0 ? `-$${Math.abs(item.price)}` : `$${item.price}`} ÷ {total} 份 = 每份 ${Math.round(item.price / Math.max(1, total))}
                            </p>
                        </div>
                        <button onClick={() => setPortionItemId(null)} aria-label="關閉" title="關閉" className="p-2 text-slate-400 hover:text-slate-600 bg-white rounded-full border border-slate-200 flex-shrink-0"><X size={18}/></button>
                    </div>
                    <div className="p-4 overflow-y-auto flex-1 space-y-2">
                        <p className="text-xs text-slate-400 mb-2">有人吃兩份就把份數加上去，不用把這道菜輸入兩次。</p>
                        {sharers.map(uid => {
                            ...
                            return (
                                <div key={uid} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-white">
                                    ...
                                    <div className="flex items-center gap-1 flex-shrink-0">
                                        <button onClick={() => setItemUnits(item.id, uid, n - 1)} disabled={n <= 1} aria-label="減少份數"
                                        ...
                                        <button onClick={() => setItemUnits(item.id, uid, n + 1)} aria-label="增加份數"
                                            className="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 font-bold hover:bg-slate-200">+</button>
                                    </div>
                                </div>
                            );
                        })}
                        {sharers.length === 0 && <p className="text-sm text-slate-400 text-center py-4">這道菜還沒有人分攤。</p>}
                    </div>
                    <div className="p-4 border-t border-slate-100">
                        <button onClick={() => setPortionItemId(null)} className="w-full py-3 bg-slate-800 text-white rounded-xl font-bold">完成</button>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4" onClick={() => setShowTreatPanel(false)}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b border-slate-100 bg-amber-50 flex justify-between items-start">
                    <div>
                        <h3 className="font-bold text-lg text-slate-800">🎁 安排請客</h3>
                        <p className="text-xs text-amber-700 mt-1">選一個人，再決定誰要請他</p>
                    </div>
                    <button onClick={() => setShowTreatPanel(false)} aria-label="關閉" title="關閉" className="p-2 text-slate-400 hover:text-slate-600 bg-white rounded-full border border-slate-200 flex-shrink-0"><X size={18}/></button>
                </div>
                <div className="p-4 overflow-y-auto flex-1 space-y-2">
                    {users.map(u => {
                        ...
                        return (
                            <div key={u.id} className={`flex items-center gap-3 p-3 rounded-xl border ${d.isTreated ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-200'}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="font-bold text-slate-700 truncate">{u.name}</div>
                                    <div className="text-[11px] text-slate-400">應付 ${d.total}</div>
                                    {d.isTreated && <div className="text-[11px] text-amber-700 truncate">🎁 {d.treatedByIds.map(userName).join('、')} 請客</div>}
                                </div>
                                {d.isTreated ? (
                                    <div className="flex gap-1 flex-shrink-0">
                                        <button onClick={() => openTreatPicker(u.id)} className="px-2.5 py-1.5 rounded-lg text-xs font-bold bg-amber-100 text-amber-700 hover:bg-amber-200">改</button>
                                        <button onClick={() => cancelTreat(u.id)} className="px-2.5 py-1.5 rounded-lg text-xs font-bold bg-slate-100 text-slate-600 hover:bg-slate-200">取消</button>
                                    </div>
                                ) : (
                    ...
                    {calculation.treats.length === 0 && (
                        <p className="text-xs text-slate-400 text-center pt-2">還沒有安排任何請客。</p>
                    )}
                </div>
                <div className="p-4 border-t border-slate-100">
                    <button onClick={() => setShowTreatPanel(false)} className="w-full py-3 bg-slate-800 text-white rounded-xl font-bold">完成</button>
                </div>
            </div>
        </div>
    );
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm animate-fade-in p-4" onClick={() => setTreatTargetId(null)}>
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b border-slate-100 bg-amber-50">
                        <h3 className="font-bold text-lg text-slate-800">🎁 誰要請 {target.name}？</h3>
                        <p className="text-xs text-amber-700 mt-1">{target.name} 這餐是 <strong>${amount}</strong></p>
                    </div>
                    <div className="p-4 overflow-y-auto flex-1 space-y-3">
                        <button onClick={() => setTreatPicks(candidates.map(u => u.id))}
                            className="w-full py-2.5 rounded-xl text-sm font-bold bg-slate-800 text-white hover:bg-slate-900">大家一起請</button>
                        <div className="space-y-2">
                            ...
                            {candidates.length === 0 && <p className="text-sm text-slate-400 text-center py-4">沒有其他人可以請客</p>}
                        </div>
                        {treatPicks.length > 0 && (
                            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-800">
                                {treatPicks.length} 人一起請 → <strong>每人多付 ${per}</strong>
                            </div>
                        )}
                    </div>
                    <div className="p-4 border-t border-slate-100 flex gap-2">
                        <button onClick={() => setTreatTargetId(null)} className="px-4 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100">取消</button>
                        <button onClick={confirmTreat} disabled={treatPicks.length === 0}
                            className="flex-1 py-3 bg-amber-500 text-white rounded-xl font-bold shadow-lg shadow-amber-200 hover:bg-amber-600 disabled:bg-slate-300 disabled:shadow-none">確認請客</button>
                    </div>
                </div>
            </div>
        );
    ...
    const renderModeChooser = () => (
        <div className="space-y-4 sm:max-w-3xl sm:mx-auto animate-fade-in">
            <div className="text-center pt-2 pb-1">
                <h2 className="text-xl font-bold text-slate-800">怎麼輸入菜色？</h2>
            ...
            <button onClick={() => { setInputMode('manual'); setStep(3); }}
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">手動輸入 <ChevronRight size={18} className="text-slate-300" /></h3>
                        <p className="text-sm text-slate-500 mt-1 leading-relaxed">一道一道加，邊加邊選誰有吃。菜色不多、或是想邊點邊記的時候最順。</p>
                    </div>
                </div>
            </button>
            ...
            <button onClick={() => { setInputMode('batch'); setStep(3); setShowImportModal(true); }}
                ...
                <span className="absolute top-3 right-3 text-[10px] font-bold bg-green-500 text-white px-2 py-0.5 rounded-full">推薦</span>
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">大量匯入 <ChevronRight size={18} className="text-green-400" /></h3>
                        <p className="text-sm text-slate-500 mt-1 leading-relaxed">一次貼上整張明細。匯入後會<strong className="text-green-700">一道一道跳卡片</strong>讓你確認誰有吃，不用自己回頭一個一個點。</p>
                        <p className="text-xs text-slate-400 mt-2 flex items-center gap-1"><Lightbulb size={12} /> 可以拍帳單請 ChatGPT 轉成文字再貼過來</p>
                    </div>
                </div>
            </button>

            <div className="pt-2 flex gap-3">
                <button onClick={() => setStep(1)} className="flex-1 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100 transition-colors">上一步：改成員</button>
                {items.length > 0 && (
                    <button onClick={() => setStep(3)} className="flex-1 py-3 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">回到清單</button>
                )}
            </div>
        </div>
    );
            ...
            <div className="space-y-4 sm:max-w-xl sm:mx-auto animate-fade-in pb-28">
                ...
                <div className="pt-1">
                    <div className="flex justify-between items-end mb-2">
                        <span className="text-sm font-bold text-slate-700">這道菜誰有吃？</span>
                ...
                <div key={item.id} className="bg-white rounded-2xl shadow-lg border border-slate-100 p-5 space-y-4 animate-fade-in">
                    <div className="flex gap-2 items-stretch">
                        ...
                        <div className="flex-shrink-0 flex items-center bg-slate-50 border border-slate-200 rounded-lg px-2 focus-within:ring-2 focus-within:ring-green-400">
                            ...
                            <input type="number" step="any" inputMode="decimal" value={item.price}
                                onChange={e => updateItemPrice(item.id, e.target.value)} aria-label="金額"
                                className="w-16 bg-transparent text-right font-bold text-slate-800 outline-none py-2" />
                        </div>
                    </div>
                    <p className="text-[11px] text-slate-400 -mt-2">品名或金額認錯了？可以直接在上面改</p>

                    {/* 標籤快捷 */}
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                        <button onClick={() => updateItemSharers(item.id, 'all')}
                            className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap flex-shrink-0 transition-colors ${item.type === 'all' ? 'bg-green-500 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>全選</button>
                        {tags.map(tag => (
                    ...
                    <div className="flex items-center justify-center gap-3 text-xs text-slate-400">
                        <span>
                            目前 <strong className="text-green-600">{item.sharedBy.length}</strong> 人
                            {hasMultiUnits(item) ? <> / <strong className="text-blue-600">{itemTotalUnits(item)}</strong> 份</> : null}
                            ，每份 ${Math.round(item.price / Math.max(1, itemTotalUnits(item)))}
                        </span>
                        <button onClick={() => setPortionItemId(item.id)} className="font-bold text-slate-500 bg-slate-100 border border-slate-200 px-2 py-1 rounded hover:bg-slate-200">⚖️ 份數</button>
                    </div>
                </div>

                <button onClick={() => { if (confirm(`確定要刪掉「${item.name}」嗎？`)) { removeItem(item.id); reviewNext(); } }}
                    className="w-full py-2 text-xs font-bold text-slate-400 hover:text-red-500 flex items-center justify-center gap-1">
                    <Trash2 size={14} /> 這道不用算，刪掉
                </button>
                ...
                <div className="fixed bottom-0 left-0 right-0 w-full max-w-md sm:max-w-2xl mx-auto p-4 sm:px-6 bg-white border-t border-slate-200 z-20 pb-safe">
                    <div className="flex gap-3">
                        <button onClick={reviewPrev} disabled={reviewIndex === 0}
                            className="px-4 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100 disabled:opacity-30 transition-colors">上一道</button>
                        <button onClick={reviewNext}
                            ...
                            {isLast ? <>完成檢視 <Check size={18} /></> : <>確認，下一道 <ChevronRight size={18} /></>}
                        </button>
                    </div>
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-800 w-full max-w-md sm:max-w-2xl lg:max-w-6xl mx-auto shadow-2xl sm:shadow-xl relative pb-safe">
            <header className="bg-white p-4 sm:px-6 shadow-sm sticky top-0 z-20 flex items-center justify-between">
                <div className="flex items-center gap-2"><div className="w-8 h-8 bg-green-500 rounded-lg flex items-center justify-center text-white"><Users size={20} /></div><h1 className="font-bold text-lg tracking-tight text-slate-800">聚餐友善分帳機</h1></div>
            ...
            <main className="p-4 sm:p-6">
                ...
                <div className="space-y-4 animate-fade-in">
                    <div className="bg-green-50 p-4 rounded-lg border border-green-200 mb-2">
                        <p className="text-sm text-green-800 leading-relaxed">
                            歡迎！請設定成員並標記標籤。<br/>可以自訂 <strong>「🥦 素食」</strong>、<strong>「🍺 只喝酒」</strong>等標籤，等等只要一鍵就能只分帳給這個標籤的人！
                        </p>
                    </div>
                    <div className="flex justify-end"><button onClick={() => setShowTagManager(true)} className="text-xs font-bold text-slate-500 flex items-center gap-1 bg-slate-100 px-3 py-1.5 rounded-full hover:bg-slate-200 transition-colors"><Settings size={14}/> 管理標籤 ({tags.length})</button></div>
                    <div className="space-y-3 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-3">{users.map((user) => { const userTag = tags.find(t => t.id === user.tagId) || tags[0]; return (<div key={user.id} className="flex items-center gap-2 bg-white p-3 rounded-xl shadow-sm border border-slate-100"><button onClick={() => setEditingUser(user.id)} aria-label={`更改 ${user.name} 的標籤`} title="更改標籤" className="w-12 h-12 flex flex-col items-center justify-center bg-slate-50 rounded-lg border border-slate-200 hover:border-green-400 hover:bg-green-50 transition-all flex-shrink-0 relative group"><span className="text-xl leading-none mb-0.5">{userTag.emoji}</span><span className="text-[10px] font-bold text-slate-500 leading-none">{userTag.name}</span><div className="absolute -bottom-1 -right-1 bg-white rounded-full border border-slate-200 p-0.5 text-slate-400 group-hover:text-green-500"><Edit size={8} /></div></button><input type="text" value={user.name} onChange={(e) => updateName(user.id, e.target.value)} aria-label="成員名字" maxLength={20} className="flex-1 min-w-0 bg-transparent border-b border-transparent focus:border-green-400 outline-none px-2 py-1 font-medium text-slate-700 text-lg" placeholder="輸入名字" /><button onClick={() => removeUser(user.id)} aria-label={`刪除成員 ${user.name}`} title="刪除成員" className="p-2 text-slate-300 hover:text-red-500 flex-shrink-0"><Trash2 size={18} /></button></div>); })}</div>
                    <button onClick={addUser} className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-500 font-bold flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors"><Plus size={20} /> 新增朋友</button>
                    <div className="pt-4 sm:max-w-md sm:mx-auto"><button onClick={() => setStep(2)} className="w-full bg-green-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-green-200 hover:bg-green-700 transition-all flex items-center justify-center gap-2">下一步：輸入菜色 <ChevronRight size={20} /></button></div>
                    <div className="mt-6 flex flex-col items-center gap-3">
                        <button onClick={resetAll} className="text-xs font-bold text-slate-400 hover:text-red-500 flex items-center gap-1 px-3 py-1.5 rounded-full hover:bg-red-50 transition-colors">
                            <Trash2 size={12}/> 清空這桌，重新開始
                        </button>
                        <span className="text-xs font-medium text-slate-400 bg-slate-100 px-3 py-1 rounded-full">Designed by 王柏文</span>
                        {savedAt && <span className="text-[10px] text-slate-300">已自動儲存 · {new Date(savedAt).toLocaleTimeString()}</span>}
                    </div>
                </div>
                )}
                ...
                <div className="space-y-4 lg:space-y-0 lg:grid lg:grid-cols-[minmax(0,420px)_minmax(0,1fr)] lg:gap-6 lg:items-start animate-fade-in">
                    ...
                    <div className="space-y-3 pb-24 pt-2 lg:pt-0 min-h-[78vh] lg:min-h-0">
                        {items.length > 0 && (
                            <div className="flex justify-between items-center px-1">
                                <span className="text-xs font-bold text-slate-400">共 {items.length} 道</span>
                            ...
                            <div className="bg-red-50 border-2 border-red-200 p-3 rounded-xl flex gap-2 items-start">
                                ...
                                <div className="text-xs text-red-700 leading-relaxed">
                                    ...
                                    <div className="mt-1 text-red-600">
                                        {calculation.unassignedItems.join('、')} 目前沒有人分攤，這些金額<strong>不會</strong>算進任何人的帳單。請幫這些項目選人，或直接刪除。
                                    </div>
                                </div>
                            </div>
                        )}
                        {items.length === 0 && <div className="text-center py-10 text-slate-400"><p>還沒有點菜喔！</p></div>}
                        ...
                        <div key={item.id} className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="flex items-center gap-3">{item.sharedBy.length === 0 && <span className="text-[10px] font-bold text-red-600 bg-red-50 border border-red-200 px-1.5 py-0.5 rounded">未分配</span>}<span className={`font-bold ${item.price < 0 ? 'text-green-600' : 'text-slate-800'}`}>{item.price < 0 ? `-$${Math.abs(item.price)}` : `$${item.price}`}</span><button onClick={() => setPortionItemId(item.id)} aria-label={`調整 ${item.name} 的份數`} title="調整份數" className={`text-xs font-bold px-1.5 py-0.5 rounded border transition-colors ${hasMultiUnits(item) ? 'bg-blue-50 text-blue-600 border-blue-200' : 'text-slate-300 border-slate-200 hover:text-slate-500 hover:border-slate-300'}`}>⚖️{hasMultiUnits(item) ? ` ${itemTotalUnits(item)}份` : ''}</button><button onClick={() => removeItem(item.id)} aria-label={`刪除 ${item.name}`} title="刪除這道菜" className="text-slate-300 hover:text-red-400"><Trash2 size={16} /></button></div>
                            </div>
                            
                            {/* 簡化後的快速套用列 (按鈕顏色即代表狀態) */}
                            <div className="flex gap-2 mb-2 overflow-x-auto no-scrollbar pb-1 border-b border-slate-100 border-dashed">
                                <span className="text-[10px] text-slate-300 self-center whitespace-nowrap flex-shrink-0">分配給:</span>
                                <button 
                    ...
                    <div className="fixed bottom-0 left-0 right-0 w-full max-w-md sm:max-w-2xl lg:max-w-6xl mx-auto p-4 sm:px-6 bg-white border-t border-slate-200 flex gap-3 z-20 pb-safe">
                        <button onClick={() => setStep(2)} className="px-4 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100 transition-colors">上一步</button>
                        <button onClick={() => setStep(4)} disabled={items.length === 0} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-green-200 hover:bg-green-700 disabled:bg-slate-300 disabled:shadow-none">計算分帳</button>
                    </div>
                </div>
                )}
                ...
                <div className="space-y-6 lg:space-y-0 lg:grid lg:grid-cols-[minmax(0,380px)_minmax(0,1fr)] lg:gap-6 lg:items-start animate-fade-in pb-32">
                    <div className="bg-slate-800 text-white p-6 rounded-2xl shadow-lg lg:sticky lg:top-[88px]">
                        <div className="flex justify-between items-end mb-4"><span className="text-slate-400">總金額 (含服務費)</span><span className="text-4xl font-bold">${Math.round(calculation.grandTotal)}</span></div>

                        <div className="bg-slate-700/50 p-3 rounded-lg space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="flex items-center gap-2 text-sm"><CalculatorIcon size={16} /> 服務費率</span>
                                <div className="flex items-center gap-2">
                                    <button onClick={() => setRate(serviceChargeRate - 1)} aria-label="服務費率減 1%" className="w-7 h-7 bg-slate-600 rounded flex items-center justify-center active:scale-95">-</button>
                                    <div className="flex items-center bg-slate-800 rounded px-2">
                                        <input type="number" step="any" inputMode="decimal" min="0" max="100" aria-label="服務費率" value={serviceChargeRate}
                                            onChange={e => setRate(e.target.value)}
                                            className="w-12 bg-transparent text-right font-mono outline-none py-1" />
                                        <span className="text-slate-400 text-sm">%</span>
                                    </div>
                                    <button onClick={() => setRate(serviceChargeRate + 1)} aria-label="服務費率加 1%" className="w-7 h-7 bg-slate-600 rounded flex items-center justify-center active:scale-95">+</button>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between border-t border-slate-600 pt-3">
                                <span className="text-sm">
                                    固定加收
                                    <span className="block text-[10px] text-slate-400">清潔費等，全員均分、不加服務費</span>
                                </span>
                                <div className="flex items-center bg-slate-800 rounded px-2">
                                    <span className="text-slate-400 text-sm">$</span>
                                    <input type="number" step="any" inputMode="decimal" aria-label="固定加收金額" value={extraFee}
                        ...
                        <div className="mb-4 bg-slate-700/50 p-3 rounded-lg">
                            <div className="text-sm mb-2 flex items-center gap-2 flex-wrap">💳 這桌誰先付？<span className="text-[10px] text-slate-400">會寫進複製的結算單</span></div>
                            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                                <button onClick={() => setPayerId(null)} className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap flex-shrink-0 ${payerId === null ? 'bg-white text-slate-800' : 'bg-slate-600 text-slate-300 hover:bg-slate-500'}`}>不指定</button>
                                {users.map(u => (
                        ...
                        <button onClick={copyAllResults} className={`w-full mt-4 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${allCopied ? 'bg-green-500 text-white' : 'bg-white text-slate-800 hover:bg-slate-100'}`}>{allCopied ? <><Check size={18}/> 已複製全員結算單！</> : <><FileText size={18}/> 複製全員結算單</>}</button>
                    ...
                    <div className="space-y-4">
                        ...
                        <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider px-2">詳細分帳結果</h3>
                                    ...
                                    <div className="flex items-center gap-3 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            {data.isTreated
                                                ...
                                                : <p className="text-xs text-slate-400">{isExpanded ? '點擊收合' : '點擊查看算式'}</p>}
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4 space-y-3">
                                        <div className="bg-slate-50 rounded-lg p-3 space-y-2 text-sm text-slate-600">
                                            {data.details.map((item, idx) => (
                                                <div key={idx} className="flex justify-between items-center py-1 border-b border-slate-200 last:border-0 border-dashed">
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="text-xs text-slate-400 font-mono">{item.originalPrice < 0 ? `-$${Math.abs(item.originalPrice)}` : `$${item.originalPrice}`} ÷ {item.totalUnits === item.sharersCount ? `${item.sharersCount}人` : `${item.totalUnits}份`}{item.myUnits > 1 ? ` ×${item.myUnits}` : ''}</div>
                                                    </div>
                                            ...
                                            <div className="pt-2 mt-2 border-t border-slate-300 space-y-1">
                                                <div className="flex justify-between text-xs text-slate-500"><span>餐費小計</span><span>${data.base}</span></div>
                                                <div className="flex justify-between text-xs text-slate-500"><span>服務費 ({serviceChargeRate}%)</span><span>${data.serviceCharge}</span></div>
                                                {data.extra !== 0 && <div className="flex justify-between text-xs text-slate-500"><span>固定加收</span><span>${data.extra}</span></div>}
                                                <div className="flex justify-between font-bold text-slate-800 text-base pt-1 border-t border-slate-300"><span>應付總額</span><span>${data.total}</span></div>
                                                {data.isTreated && (
                                        ...
                                        {data.isTreated ? (
                                            <div className="flex gap-2">
                                                <button onClick={(e) => { e.stopPropagation(); openTreatPicker(u.id); }} className="flex-1 py-2 rounded-lg text-sm font-bold bg-amber-100 text-amber-700 hover:bg-amber-200">改請客的人</button>
                                                <button onClick={(e) => { e.stopPropagation(); cancelTreat(u.id); }} className="flex-1 py-2 rounded-lg text-sm font-bold bg-slate-100 text-slate-600 hover:bg-slate-200">取消請客</button>
                                            </div>
                                        ) : (
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); copyToClipboard(u.id, u.name, data); }} className={`w-full py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors ${copiedId === u.id ? 'bg-green-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                                            {copiedId === u.id ? <><Check size={16}/> 已複製單人明細</> : <><Copy size={16}/> 複製單人算式</>}
                                        </button>
                                    </div>
                                </div>
                                )}
                    ...
                    <div className="fixed bottom-0 left-0 right-0 w-full max-w-md sm:max-w-2xl lg:max-w-6xl mx-auto p-4 sm:px-6 bg-white border-t border-slate-200 pb-safe">
                        <div className="flex gap-3">
                            <button onClick={() => setStep(3)} className="flex-1 bg-slate-100 text-slate-600 py-3 rounded-xl font-bold hover:bg-slate-200">返回修改</button>
                            <button onClick={resetAll} className="px-4 py-3 rounded-xl font-bold text-slate-500 border border-slate-200 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors whitespace-nowrap">開新的一桌</button>
                        </div>
                    </div>
                </div>
                )}
            ...
            {undoInfo && (
                <div className="fixed left-0 right-0 bottom-24 z-40 flex justify-center px-4 pointer-events-none">
                    <div className="pointer-events-auto bg-slate-800 text-white rounded-xl shadow-2xl px-4 py-3 flex items-center gap-4 max-w-sm w-full animate-fade-in">
                        ...
                        <button onClick={applyUndo} className="text-sm font-bold text-green-400 hover:text-green-300 flex-shrink-0">復原</button>
                        <button onClick={() => setUndoInfo(null)} aria-label="關閉提示" className="text-slate-400 hover:text-white flex-shrink-0"><X size={16}/></button>
                    </div>
                </div>
            )}
            {copyFallbackText && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[80vh]">
                        <div className="p-4 border-b border-slate-100 bg-amber-50">
                            <h3 className="font-bold text-slate-800">自動複製失敗 😢</h3>
                            <p className="text-xs text-slate-500 mt-1">請長按下面的文字全選後手動複製。</p>
                        </div>
                        ...
                        <div className="p-4 pt-0">
                            <button onClick={() => setCopyFallbackText('')} className="w-full py-3 bg-slate-800 text-white rounded-xl font-bold">關閉</button>
                        </div>
                    </div>
                </div>
            )}

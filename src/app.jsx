...
const BillSplitter = () => {
    ...
    const copyAllResults = () => {
        ...
        text += ')\n';
        // 誰請誰：列在轉帳指示上面
        if (calculation.treats.length > 0) {
            text += '\n';
            calculation.treats.forEach(tr => {
                text += isEn ? `🎁 ${nameList(tr.payerIds)} treated ${userName(tr.beneficiaryId)}\n`
                             : `🎁 ${nameList(tr.payerIds)} 請 ${userName(tr.beneficiaryId)}\n`;
            });
        }
        if (payerId && calculation.userResults[payerId]) {
            text += isEn ? `\n💳 ${userName(payerId)} paid — please transfer your share to them\n`
        ...
        text += isEn ? `\n${rule}\nGenerated by Meal Splitter · Designed by 王柏文\n👉 Try it: ${APP_URL}`
